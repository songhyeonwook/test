...
class GlobalLocalizationNode(Node):
    ...
    def cb_init_pose(self, msg: PoseWithCovarianceStamped):
        ...
        # /initialpose is a pose of the robot body in the map frame.
        # ICP, however, estimates map -> camera_init (odom) because current scans
        # are represented in the camera_init frame. Convert the clicked body pose
        # into an initial map->camera_init guess using the current odometry.
        T_map_body = self.pose_to_mat(msg)
        T_cam_body = self.pose_to_mat(self.cur_odom)
        initial_map_to_odom = T_map_body @ np.linalg.inv(T_cam_body)
